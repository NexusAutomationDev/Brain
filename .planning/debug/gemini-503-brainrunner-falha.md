...
hypothesis: O retry JÁ EXISTE e já está exaurindo (7 tentativas / ~87s). A causa da
  "lead sem resposta" NÃO é ausência de retry, e sim (a) modelo saturado sem
  caminho de degradação (nenhum fallback de modelo) + (b) catch terminal que só
  loga e devolve HTTP 500 sem produzir resposta ao lead.
test: probe empírico do AsyncCaller com erro 503 no formato exato de produção
expecting: se retry existe, o tempo total deve bater com os gaps do log de produção
next_action: aplicar fix em packages/ai — retry configurável + cadeia de fallback de modelo
...
fix: |
  ...
  `LLM_FALLBACK_MODELS` no deploy do brain-sdr — ex.:
  `LLM_FALLBACK_MODELS=gemini-2.5-flash` (o modelo que comprovadamente funcionava).
